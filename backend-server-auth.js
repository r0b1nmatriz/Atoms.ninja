#!/usr/bin/env node
// Updated Backend Server with Google Authentication
...
const { GoogleAuthManager, setupAuthRoutes } = require('./auth-manager');
...
const authManager = new GoogleAuthManager({
  ...
  redirectUri: process.env.GOOGLE_REDIRECT_URI || 'http://136.113.58.241:3002/auth/google/callback'
});
...
setupAuthRoutes(app, authManager);
...
app.post('/api/multi-ai', authManager.requireAuth.bind(authManager), async (req, res) => {
  try {
    ...
    // Log user activity
    console.log(`📝 User ${req.session.user.email} - AI request: ${message.substring(0, 50)}...`);
...
// Kali MCP proxy (protected)
app.post('/api/kali', authManager.requireAuth.bind(authManager), async (req, res) => {
  try {
    ...
    console.log(`🔧 User ${req.session.user.email} - Tool: ${cmd}`);
...
// OpenAI direct endpoint (protected)
app.post('/api/openai', authManager.requireAuth.bind(authManager), async (req, res) => {
  try {
...
// Gemini direct endpoint (protected)
app.post('/api/gemini', authManager.requireAuth.bind(authManager), async (req, res) => {
  try {
...
app.use((req, res) => {
  res.status(404).json({ 
    ...
    availableEndpoints: [
      ...
      'GET /auth/google',
      'GET /auth/status',
      'POST /api/multi-ai (protected)',
      'POST /api/kali (protected)',
      'POST /api/openai (protected)',
      'POST /api/gemini (protected)'
    ]
  });
});
